dbnam='pngbodydb';
dbdir='/Volumes/Portadb/PORTAPNGBODY/pngbodydb_tables/';
datadir='/Volumes/Portadb/PORTAPNGBODY/pngbodydb_tables/data/';
nulldir='/Volumes/Portadb/PORTAPNGBODY/pngbodydb_tables/data/dead/';
...
mkdir(strcat(nulldir,'/',D(id).name));
...
ofile   =strcat(nulldir,D(id).name,'/',F(ifl).name);
...
evtime=str2epoch(sprintf('%u/%u %u:%u:%u',yyyy,jjj,hh,mm,ss));
...
if nrec==0 
	...
	movefile(fullfile,strcat(nulldir,'/',D(id).name));
