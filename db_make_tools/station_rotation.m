%STATION_ROTATION
%  Do all the rotations for all the OBS/misaligned data to get into proper Z-N-E form
%  Input:   needs portapng db  with .wfdisc .site .sitechan  tables
...
%	Z. Eilon    23 May 2013
%   Z. Eilon     1 Jul 2013
fprintf('NB must edit so marks wfdisc rows and deletes them. Could even have it merge back in in this script/n')
...
scors = [ 116.8  0.3  183.1  301  47.2  324.5  143.9];
%OBS:   [ 116.8  0.3  183.1  301  47.2  324.5  143.9]; 
%Land:  [15.6  12.1  21.4  17.2]; 

dbnam='pngbodydb_plus';
dbout='pngbodydb_plus_rot';
...
%% XXXXXXXXXXXXXXXXXXXXX No edits needed below here XXXXXXXXXXXXXXXXXXXXX
%% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
dbsch.record = dbfind(dbsch,sprintf('sta == "%s" && chan == "%s"',sta,ochans(1,:)));
...
dbsch.record = dbfind(dbsch,sprintf('sta == "%s" && chan == "%s"',sta,ochans(2,:)));
chanids(2) = dbgetv(dbsch,'chanid');
dbputv(dbsch,'hang',0.0,'vang',90.0)
end
%VERTICAL
dbsch.record = dbfind(dbsch,sprintf('sta == "%s" && chan == "%s"',sta,ochans(3,:)));
